function Sync-FGCSVPrincipalActivity {
    <#
    .SYNOPSIS
    Imports principal activity data from a CSV file into the PrincipalActivity table.

    .DESCRIPTION
    Loads activity data exported from any external system (HR system, SIEM, PAM tool, etc.)
    into the PrincipalActivity table. This enables role mining signals from systems that
    do not have a native Graph API integration.

    CSV columns (semicolon-delimited):
    - principalId       (required) GUID matching a Principals.id value
    - resourceId        (optional) GUID matching a Resources.id value; leave empty for general activity
    - activityType      (optional) Default: 'ExternalSignIn'. Suggested: 'SignIn', 'ResourceAccess', 'Privileged'
    - lastActivityDateTime (required) ISO 8601 datetime string
    - activityCount     (optional) Number of events in the period
    - periodStart       (optional) ISO 8601 start of aggregation window
    - periodEnd         (optional) ISO 8601 end of aggregation window
    - extendedAttributes (optional) JSON string for additional context
    ...
    Activity type to use when the CSV row does not specify one. Default: 'ExternalSignIn'.

    .EXAMPLE
    Sync-FGCSVPrincipalActivity -FilePath ".\exports\activity.csv"

    .EXAMPLE
    Sync-FGCSVPrincipalActivity -FilePath ".\sap_logins.csv" -DefaultActivityType "SAPSignIn"
    ...
    [CmdletBinding()]
    [Alias("Sync-CSVPrincipalActivity")]
    Param(
        ...
        [Parameter(Mandatory = $false)]
        [string]$DefaultActivityType = 'ExternalSignIn'
    )
...
"@
    ...
    Write-Host "CSV Principal Activity Import Complete!" -ForegroundColor Green
    ...
    return @{
        ...
        Write-FGSyncLog -SyncType "CSVPrincipalActivity" -StartTime $syncStartTime -RecordCount $syncRecordCount -Status $syncStatus -ErrorMessage $syncErrorMessage -TableName "PrincipalActivity"
    }
}
