...
"@
        ...
        $syncResult = Invoke-FGSQLCommand -ScriptBlock {
            ...
            try {
                ...
                # Scoped delete: only remove Direct memberships not in source
                ...
                $deleteCmd.CommandText = "CREATE TABLE #SyncSourceMemberIds (resourceId UNIQUEIDENTIFIER, principalId UNIQUEIDENTIFIER, PRIMARY KEY (resourceId, principalId))"
                ...
                $bulkCopy.DestinationTableName = "#SyncSourceMemberIds"
                $bulkCopy.WriteToServer($idTable)
                $bulkCopy.Close()

                $deleteCmd.CommandText = "DELETE t FROM dbo.[$tableName] t LEFT JOIN #SyncSourceMemberIds s ON t.resourceId = s.resourceId AND t.principalId = s.principalId WHERE s.resourceId IS NULL AND t.assignmentType = 'Direct'"
                $deletedCount = $deleteCmd.ExecuteNonQuery()

                $deleteCmd.CommandText = "DROP TABLE #SyncSourceMemberIds"
                $deleteCmd.ExecuteNonQuery() | Out-Null
                $deleteCmd.Dispose()
                $idTable.Dispose()
