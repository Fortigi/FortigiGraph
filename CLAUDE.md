...
- **Risk Scoring Page**: Visualize identity risk scores — tier badges, score bars, per-layer breakdown, classifier match details, analyst override controls
- **Org Chart Page**: Manager hierarchy tree with risk propagation — hybrid layout, department drill-down, risk-colored department boxes
- **Department Detail Pages**: Deep dive into department risk profiles with member attribution and subtree aggregates
- **Governance Page**: Access package review compliance monitoring — KPIs, per-AP review status, category filtering (currently disabled)
