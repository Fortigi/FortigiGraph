...
- **Authentication settings UI** — new Admin → Authentication sub-tab lets the operator configure Entra ID SSO from the browser instead of editing environment variables and restarting containers. The settings (`AUTH_ENABLED`, `AUTH_TENANT_ID`, `AUTH_CLIENT_ID`, `AUTH_REQUIRED_ROLES`) now live in the existing `dbo.WorkerConfig` table; new module [app/api/src/config/authConfig.js](app/api/src/config/authConfig.js) loads them at startup with env-var fallback for backward compatibility, and exposes a `reloadAuthConfig()` that the admin save endpoint calls so the JWT validation middleware picks up new values without a container restart. The middleware ([app/api/src/middleware/auth.js](app/api/src/middleware/auth.js)) was rewritten to read from this dynamic state instead of static `process.env`. New endpoints: `GET /api/admin/auth-settings` (current snapshot — no secrets, MSAL is a public-client flow), `PUT /api/admin/auth-settings` (validates GUIDs, persists, reloads). The new [AuthSettingsPage.jsx](app/ui/src/components/AuthSettingsPage.jsx) bundles a five-step Entra app registration walkthrough with copy-to-clipboard for the auto-detected redirect URI (`window.location.origin` so multi-domain deployments just work — register each domain as a separate redirect URI in the same Entra app), an `api://<clientId>/access` scope hint, GUID validation on the inputs, an optional comma-separated required-roles field, and a prominent lockout warning with a recovery SQL command (`UPDATE dbo.WorkerConfig SET configValue = 'false' WHERE configKey = 'AUTH_ENABLED'`) so an operator who locks themselves out has a clear escape route. Auth is still off by default; the page is reachable without authentication when auth is disabled (which is the only way to enable it for the first time).
