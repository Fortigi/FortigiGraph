...
- Added Admin page to the Role Mining UI with data export/import functionality — enables migration of manually-curated data (tags, categories, risk overrides, identity verifications, member overrides, cluster owners) between environments as a JSON bundle. Tags and categories are matched by name on import, so IDs don't need to match. The Admin tab is optional and can be enabled via the user settings dropdown
