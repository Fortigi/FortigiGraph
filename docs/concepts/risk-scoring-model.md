...
| Temporal | **Yes** — full score history retained |
...
`RiskScores` is the only risk table that uses SQL Server temporal versioning. This means every score change — whether from a re-scoring run or an analyst override — is preserved in `RiskScoresHistory`.

```sql
-- Full score history for a principal
SELECT rs.riskScore, rs.riskTier, rs.riskOverride, rs.riskOverrideReason,
       rs.ValidFrom, rs.ValidTo
FROM RiskScores FOR SYSTEM_TIME ALL
WHERE entityId = 'your-principal-guid'
  AND entityType = 'Principal'
ORDER BY rs.ValidFrom DESC;

-- All entities that moved from High to Critical in the last 30 days
SELECT rs.entityId, rs.entityType, rs.riskScore, rs.ValidFrom
FROM RiskScores FOR SYSTEM_TIME ALL
WHERE riskTier = 'Critical'
  AND ValidFrom >= DATEADD(DAY, -30, GETDATE())
  AND NOT EXISTS (
      SELECT 1 FROM RiskScores FOR SYSTEM_TIME AS OF DATEADD(DAY, -30, GETDATE()) prev
      WHERE prev.entityId = rs.entityId AND prev.riskTier = 'Critical'
  );
```

The other risk tables (`GraphRiskProfiles`, `GraphRiskClassifiers`, `GraphCorrelationRulesets`, `GraphResourceClusters`, `GraphResourceClusterMembers`) are not temporal — they are overwritten in place when regenerated.
